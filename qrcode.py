...
def generateCode():
  ...
  s = text.get() #Adding the data to be encoded to the QRCode object
  url = pyqrcode.create(s)  #Making the entire QR Code space utilized
  #Generating the QR Code
  fileDirec=loc.get()+'\\'+name.get() #Getting the directory where the file has to be save
  url.svg(f'{fileDirec}.svg', scale = 8)
  url.png(f'{fileDirec}.png', scale = 6) #Saving the QR Code
